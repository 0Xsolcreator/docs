...
Here we added methods to extract the input message metadata that we passed for signing and the output that we go from MWA and 
transform it in the form that we can pass to the `verifySignIn` method from `@solana/wallet-standard-util` package to verify the signed message.
...
Though the flow is correct, for some reason the `verifySignIn` method is returns false on every occasion. But the same message can we verified
